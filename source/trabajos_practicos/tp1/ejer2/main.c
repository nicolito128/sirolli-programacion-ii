...
// Cabeceras del menú y sus opciones
void menu();
...
// No pertenece al ejercicio, pero es útil mientras se está desarrollando.
// Muestra el conjunto por consola, pero en lugar del caracter muestra el código numérico que lo identifica.
void mostrar_codes(Conjunto c) {
    printf("\n{ ");
...
void menu() {
    ...
    do {
        printf("\n--- Menú para el Ejercicio 2: Conjuntos ---");
        printf("\n[ 0 ] Salir.");
...
Conjunto generar_conjunto() {
    ...
    if (tam < 0) {
        printf("\nEl tamaño del conjunto debe ser un número positivo.");
        gen.cant = 0; // Conjunto vacio en caso de error
        return gen;
    }
...
void opcion_mostrar_union(Conjunto a, Conjunto b) {
    ...
    opcion_mostrar_conjunto(union_ab, "A unión B");
}
...
void opcion_determinar_pertenencia(Conjunto a, Conjunto b) {
    ...
    if (pertenece_a == 1 && pertenece_b == 1) {
        ...
        printf("- No pertenece a ningún conjunto.\n");
    }
}