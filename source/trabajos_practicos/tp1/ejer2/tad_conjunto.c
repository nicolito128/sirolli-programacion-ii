...
Conjunto inicializar(int n) {
	...
	i = 0;
	while (i < n) {
		// Generamos un caracter ASCII aleatorio. Es decir, un numero entre 0 y 127.
		ch = rand() % 128;
...
Conjunto agregar(Conjunto c, char ch) {
	// Siempre que no pertenezca y no exceda la capacidad maxima, lo agregamos.
	if (pertenece(c, ch) == 0 && (c.cant+1) <= CAP_CONJUNTO) {
		// Utilizamos el algoritmo de ordenamiento por insercion.
		int i = c.cant - 1;
...
/*
Debido a que el conjunto esta ordenado ascendentemente,
...
Por lo tanto, solo nos queda salir del bucle ya que `ch` no
pertenece al conjunto.

Esta optimizacion evita que recorramos la totalidad del
